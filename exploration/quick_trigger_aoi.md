...
```python
adm2_aoi_pcodes = adm2[adm2["ADM1_PCODE"].isin(ADM1_AOI_PCODES)][
    "ADM2_PCODE"
].unique()
```
...
adm2[adm2["ADM2_PCODE"].isin(adm2_aoi_pcodes)].plot(ax=ax, color="dodgerblue")
ax.axis("off")
```
...
```python
df_exp_aoi = df_exp[df_exp["ADM2_PCODE"].isin(adm2_aoi_pcodes)]
```
...
```python
df_exp_aoi = df_exp_aoi.merge(df_storms)
```

```python
df_exp_recent = df_exp_aoi[df_exp_aoi["season"] >= 2001]
```
...
display(
    ...
    .background_gradient(subset=wind_col, cmap=light_oranges, vmax=271_731)
